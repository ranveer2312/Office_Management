...
interface EmployeePayrollDTO {
  ...
  effectiveWorkDays: number; // For the current month/period (mocked or from attendance)
}
// ------------------------------------------------------------------------

const API_BASE_URL = 'http://localhost:8080';

// Month-Year Picker Component (Remains the same)
...
const HRPayslipManagerPage: React.FC = () => {
  ...
  // UPDATED FETCH LOGIC: Fetches from the new dedicated endpoint
  const fetchEmployees = useCallback(async () => {
    setLoading(true);
    try {
      // Fetch data from the new payroll-list endpoint
      ...
      setStatus(`Error loading employee list: ${error.message || 'Unknown error'}`);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  // UPDATED GENERATION LOGIC: Targets the generate-archive endpoint
  const handleGeneratePayslip = async (employeeId: string | null) => {
    ...
    try {
      // Use the new /generate-archive endpoint for processing (PDF creation + DB save)
      ...
      // Response contains the success message from the backend
      ...
      setStatus(`Generation failed for ${actionText}: ${error.response?.data || error.message || 'Unknown error'}`);
    } finally {
  ...
  const getStatusStyle = () => {
    if (status?.includes('Error') || status?.includes('failed')) return { icon: AlertTriangle, color: 'text-red-600', bg: 'bg-red-50', border: 'border-red-300' };
    ...
    <div className="p-8 max-w-7xl mx-auto bg-white min-h-screen shadow-xl rounded-xl">
      ...
      <section className="mb-10 p-6 border border-gray-200 rounded-xl bg-gray-50 shadow-md">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between space-y-4 md:space-y-0">
          ...
          <button
            // Pass null to trigger the backend's bulk generation logic
